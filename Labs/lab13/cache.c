	...
	while(fgets(trace,sizeof(trace),stdin)) {
		...
		if (2==sscanf(trace,"%c %p ",&tt,&taddr)) {
			...
			switch(cache_type) {
				case 'F':
					/*--------------------------------------------------------------------------------------
						Need to figure out if there is already a way set up for this tag by checking for 
							the tag in the cam.
						If the way is not there, or if the way is not valid, then you will need to...
							- find a victim - use the LRU algorithm in way_firstUsed()
							- Update the CAM to write the tag to the victim way
							- Invoke the way_setValid() function for the victim
						when done, the "way" variable should contain the way index of the way which matches
							 the current tag.
					-----------------------------------------------------------------------------------------*/

					/* Your code goes here... */
					way = cam_read(f_cam,tag);
					if(!way_isValid(ways,way) || way == -1){
